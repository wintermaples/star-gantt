/**
 * Sample task data and a recording `PluginContext`, for tests that exercise a plugin's `setup()`
 * without booting a real chart.
 */
...
/** A flat list of `n` root tasks named `t0…`, each one day long, starting at the epoch. */
export function flatTasks(n: number): Task[] {
  const out: Task[] = [];
  for (let i = 0; i < n; i += 1) {
    out.push({ id: `t${i}`, parentId: null, name: `t${i}`, start: 0, end: MS_DAY });
  }
  return out;
}

/* ------------------------------------------------------------------ *
 * A recording PluginContext
 * ------------------------------------------------------------------ */
...
export interface FakeContext {
  ...
  /** Disposes everything the plugin handed to `ctx.own()`, newest first. */
  releaseOwned(): void;
}
...
/**
 * A recording `PluginContext` for testing a plugin's `setup()` in isolation — no kernel, no
 * sibling plugins.
 *
 * Every call is logged, `use()` throws for a service that was not supplied (mirroring the
 * "declared dependency only" rule closely enough to catch a missing stub), and `useOptional()`
 * returns `undefined` instead. Extension-point contributions are recorded but never reduced: the
 * point owner is not present, so the test asserts on the contributions themselves.
 */
export function fakePluginContext(options: FakeContextOptions = {}): FakeContext {
  ...
  return {
    ...
    releaseOwned(): void {
      for (let i = log.owned.length - 1; i >= 0; i -= 1) log.owned[i]?.dispose();
      log.owned.length = 0;
    },
  };
}
