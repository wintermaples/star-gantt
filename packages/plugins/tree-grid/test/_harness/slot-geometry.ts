 ...
 * it declares.
 *
 * The fake DOM has no CSS engine, so `calc(var(--sg-safe-right, 0px) + 12px)` is only a string
 * there and an assertion on that string proves nothing. These helpers do what a browser would:
 * read the four `--sg-safe-*` lengths the renderer publishes on the chart pane, resolve the
 * declaration — `calc()`, `min()`, `max()`, `var()` with its fallback, and `%` of a caller-supplied
 * pane box — against them, and hand back pixel offsets a test can compare with the safe area
 * itself. Real layout — the pane's clamped width at the viewport floor, text measurement, wrapping
 * — is verified elsewhere; what is proven here is the arithmetic every one of those layouts starts
 * from.
 */
import type { DomHarness } from "./dom-install";
import type { FakeElement } from "./dom-element";
 ...
 * safe-area writer actually publishes `--sg-safe-*` onto the chart pane. Must run before the chart
 * is created: the renderer creates its pane, and publishes for the first time, inside its own
 * setup.
 *
 ...
 * string it carries.
 */
function resolveCssPx(decl: string, pane: FakeElement, basis: number): number {
 ...
 * px, against the pane box a percentage resolves against. Every field is optional — nothing is
 * assumed about which sides the caller's overlay anchors.
 */
